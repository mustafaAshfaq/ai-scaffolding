# orchestrate
Breaks requests into phases and delegates to specialist subagents in parallel.
use claude-opus-4.7[] to manage orchestration and task.
## Agents
...
### Step 0: Create the task directory
Before any agent call, create a new task directory by copying `tasks/_template/` to `tasks/YYYY-MM-DD-<kebab-slug>/`. Fill in the `README.md` (Title, Status=`planning`, Target project, Request). All planner / designer / coder artifacts for this request live inside that directory; agents must not write to the repo root.

### Step 1: Get the Plan
Call the Planner agent with the user's request **and the task directory path**. The Planner will write its output into `<task-dir>/plans/` and `<task-dir>/specs/` and return implementation steps with file assignments.
...
After all phases complete, verify the work hangs together, update the task `README.md` Status to `done` (or `blocked`) and append a final Changelog entry, then report results to the user with a link to the task directory.
